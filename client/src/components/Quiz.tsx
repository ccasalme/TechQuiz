...
//   // Inside your component's return statement, handle error state:
//   if (error) {
//     return (
//       <div className="alert alert-danger text-center" role="alert">
//         {error}
//         <button className="btn btn-primary mt-2" onClick={getRandomQuestions}>Retry</button>
//       </div>
//     );
//   }
  ...
  return (
    <div className='quiz-container'>
      ...
      <div className="mt-3">
        {currentQuestion.answers.map((answer, index) => (
          <div key={index} className="d-flex align-items-center mb-2">
            <button className={`btn btn-primary answer`} onClick={() => handleAnswerClick(answer.isCorrect)}>{index + 1}</button>
            <div className="alert alert-secondary mb-0 ms-2 flex-grow-1">{answer.text}</div>
          </div>
        ))}
      </div>
